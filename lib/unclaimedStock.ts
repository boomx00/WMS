...
// How much of an item, currently sitting in Outbound WH, has no SO AND no
// Tambahan attached — either it was never claimed (excess from a picking
// action that overshot what its SO needed), or it was released back by a
// Finish action. CLAIM events remove from this pool as SOs draw on it.
// Stock picked under a Tambahan is excluded here even though it has no
// salesOrderId — it's earmarked to that Tambahan, not free-floating.
  ...
  const [untaggedPicked] = await db
    ...
    .where(
      and(
        ...
        isNull(locationStockEvents.tambahanOrderId),
