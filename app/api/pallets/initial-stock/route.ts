...
const result = await db.transaction(async (tx) => {
  const [existing] = await tx
    .select()
    .from(pallets)
    .where(and(eq(pallets.label, item.defaultCode), eq(pallets.locationId, location.id)));
  ...
  if (existing && existing.status === "ACTIVE") {
    ...
    [pallet] = await tx
      .update(pallets)
      .set({ status: "ACTIVE", quantity, removedAt: null, updatedAt: new Date() })
      .where(eq(pallets.id, existing.id))
      .returning();
  } else {
    [pallet] = await tx
  ...
  await adjustLocationStock(tx, location.id, item.id, quantity);
