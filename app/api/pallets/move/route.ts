...
import { moveLocationStock, adjustLocationStock } from "@/lib/locationStock";
...
await moveLocationStock(tx, currentLocation.id, newLocation.id, exactPallet.itemId, exactPallet.quantity);
  ...
  if (item) {
    ...
    if (sourceBucket) {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        await moveLocationStock(tx, currentLocation.id, newLocation.id, item.id, quantity);
        return destinationPallet;
      });
...
await tx.insert(palletEvents).values({
  ...
  type: "DEFAULT_INBOUND",
  locationId: currentLocation.id,
  userId: session.userId,
  quantity,
});
        await adjustLocationStock(tx, currentLocation.id, autoItem.id, quantity);

        await tx.insert(locationStockEvents).values({
          type: "DEFAULT_INBOUND",
          itemId: autoItem.id,
          sourceLocationId: null,
          destinationLocationId: currentLocation.id,
          quantity,
          userId: session.userId,
        });
        ...
        await moveLocationStock(tx, currentLocation.id, newLocation.id, autoItem.id, quantity);
